...
Mat Aia5::calcCompLogL(const vector<struct comp*>& model, const Mat& features){
	// To Do !!!
	int C_current = model.size();	
	int NumberFeatures = features.cols;	// Number of features/samples (x)
	int NumberDimensions = features.rows;	// Number of dimension of the vectors (x_i)

	// Creating a Mat containing the probabilities log(p(x_i|mu_c,Cov_c)) that are to be
	// the output.
	Mat logPout = Mat::zeros(C_current,NumberFeatures,CV_32FC1);
	float Z = -NumberDimensions/2*log(CV_PI);
	Mat Ones = Mat::ones(NumberDimensions,NumberFeatures,CV_32FC1);
	...
	for(int i = 0; i < C_current; i++){
		...
		float Constant = -0.5*log(determinant(model.at(i)->covar));
			...
			for(int j = 0; j < Number_features; j++){
				...
				Mat vecDiff = features.col(j)-model.at(i)->mean;	// Defining the difference vector (x_i-y_c)
				Mat MatrixMul = -0.5*(vecDiff.t()*inv_M*vecDiff) +  Constant - Z; 

				logPout.at<float>(i,j) = MatrixMul.at<float>(0,0);
...
Mat Aia5::calcMixtureLogL(const vector<struct comp*>& model, const Mat& features){
	...
	int numberDimensions = features.rows;	// Number of dimension of the vectors (x_i)
        ...
        Mat logProbAllCs = calcCompLogL( model, features);	

	Mat logSum = Mat::zeros(1,numberFeatures, CV_32FC1);

	for(int j = 0; j < numberFeatures; j++){
	
		float expSum = 0;
			for(int i = 0; i < C_current; i++){
					expSum += (model.at(i)->weight)*exp(logProbAllCs.at<float>(i,j));
			}
		logSum.at<float>(0,j) = log(expSum);
		}
	return logSum;
}
