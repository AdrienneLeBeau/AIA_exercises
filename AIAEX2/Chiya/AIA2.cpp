...
Mat Aia2::normFD(const Mat& fd, int n){
  ...
  cv::Mat2d translation_invariant;
  fd.copyTo(translation_invariant);

//translation invariance
  translation_invariant(0,0) = Vec2d(0.0,0.0);
  
 //scale invariance
  float norm_positive = cv::norm(fd.at<Vec2d>(1,0));
  for(int i= 0; i< fd.rows; ++i)
  {
   translation_invariant(i,0) /= norm_positive;
  }

 // rotation invariance
  cv::Mat1d rotation_invariant(fd.rows,1,0.0);
  for(int i = 0;i < fd.rows; ++i)
  {
    rotation_invariant(i,0) = cv::norm( translation_invariant(i,0));
  }
  
  // smaller sensitivity for details
 cv::Mat1d sensitive(rotation_invariant.rows, 1, 0.0f);
 for (int i = 0; i < n / 2; ++i)
 {
	 sensitive(i, 0) = roation_invariant(i, 0);
	 sensitive(sensitive.rows - 1 - i) = roation_invariant(roation_invariant.rows - 1 - i, 0);
 }
  
 cv::Mat1d output(n, 1, 0.0f);
  for (int i = 0; i < n/2; ++i)
  {
	  output(i, 0) = rotation_invariant(i, 0);
	  output(n-1-i) = rotation_invariant(rotation_invariant.rows -1 -i, 0);
  }
